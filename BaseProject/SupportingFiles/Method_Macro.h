...
#define HideProcessHud  [weakSelf hideProcessHud];
...
#define Show_Height [UIScreen mainScreen].bounds.size.height - ([[UIApplication sharedApplication] statusBarFrame].size.height + self.navigationController.navigationBar.frame.size.height) - self.tabBarController.tabBar.frame.size.height
//nav二级页面的高度，去除了statusBar、navigationBar、bottomBar
#define Show_Height_Next [UIScreen mainScreen].bounds.size.height - ([[UIApplication sharedApplication] statusBarFrame].size.height + self.navigationController.navigationBar.frame.size.height) - ([[CommonUtil getPhoneDevicName] isEqualToString:@"iPhone X"]?34:0)
//去除bottomBar后的设备高度
#define ScreenHeight_WithoutBottomBar ([[UIScreen mainScreen] bounds].size.height - ([[CommonUtil getPhoneDevicName] isEqualToString:@"iPhone X"]?34:0))
...
//设置边缘线
#define SetViewBorder(view)   {view.layer.borderColor = [ColorWithHexString(ColorLine_e0e0e0) CGColor];  view.layer.borderWidth = 0.5;}
...
#define UserID  [UserInfo shareInstance].userID
#define UserIP  [UserInfo shareInstance].ipPort
