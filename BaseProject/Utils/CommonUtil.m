...
+ (BOOL)checkIpPort:(NSString *)string {
    if (![string containsString:@":"])
        return NO;
    NSString *regex = @"^(1\\d{2}|2[0-4]\\d|25[0-5]|[1-9]\\d|[1-9])\\.(1\\d{2}|2[0-4]\\d|25[0-5]|[1-9]\\d|\\d)\\.(1\\d{2}|2[0-4]\\d|25[0-5]|[1-9]\\d|\\d)\\.(1\\d{2}|2[0-4]\\d|25[0-5]|[1-9]\\d|\\d):([0-9]|[1-9]\\d{1,3}|[1-5]\\d{4}|6[0-5]{2}[0-3][0-5])";
    NSPredicate *pred = [NSPredicate predicateWithFormat:@"SELF MATCHES %@", AvailableString(regex)];
    BOOL isMatch = [pred evaluateWithObject:string];
    return isMatch;
}
