...
- (void)loadView
{
    ...
    _innerContainer						= [[UIView alloc] initWithFrame:CGRectMake( 0, [[UIApplication sharedApplication] statusBarFrame].size.height, _container.frame.size.width, ScreenHeight_WithoutBottomBar - [[UIApplication sharedApplication] statusBarFrame].size.height)];
...
{
    UITapGestureRecognizer *tapGes = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(leftTapGes:)];
    UIImageView *imgV = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, 10, 18)];
    imgV.image = [UIImage imageNamed:@"navi_back"];
...
- (void)leftTapGes:(UITapGestureRecognizer *)tapGes
{
...
- (void)positionInnerContainer
{
	...
	{//portrait
		innerContainerRect = CGRectMake( 0, [[UIApplication sharedApplication] statusBarFrame].size.height, _container.frame.size.width, ScreenHeight_WithoutBottomBar - [[UIApplication sharedApplication] statusBarFrame].size.height);
	}
	else 
	{// landscape
...
- (void)updateTitle
{
	[self setTitle:[NSString stringWithFormat:@"%ld/%li", (long)_currentIndex+1, (long)[_photoSource numberOfPhotosForPhotoGallery:self]]];
}
...
- (FGalleryPhoto*)createGalleryPhotoForIndex:(NSUInteger)index
{
	...
	if( sourceType == FGalleryPhotoSourceTypeLocal )  //本地图片
	{
	...
	else if( sourceType == FGalleryPhotoSourceTypeNetwork ) //网络图片
	{
...
- (void)galleryPhoto:(FGalleryPhoto*)photo didLoadThumbnail:(UIImage*)image
{
    ...
    [photoView.loadingView removeFromSuperview];
    ...
    [thumbView.loadingView removeFromSuperview];
}
...
//        [photoView.imageView setImageWithURL:URLFromId(@"") placeholderImage:[UIImage imageNamed:@"food_banner"] success:^(UIImage *sucImg){
//            NSLog(@"图片获取成功");
//        } failure:^(NSError *error){
//            NSLog(@"图片获取失败");
//        }]; //[UIImage imageNamed:@"food_banner"]
